...
log_file = open(sys.argv[3], "a")
log_file.write("Log for neighbours model\n")
log_file.close()
...
log_file = open(sys.argv[3], "a")
log_file.write("- distacnes loaded\n")
log_file.close()
...
class PVDataset(Dataset):
    def __init__(self, X,
               df_distances, look_up=1, look_ahead=1,
...
def loaders(xr_test, xr_train, batch_size, dict_look_up, dict_look_ahead,
            ...
            df_distances_train, df_distances_test,
            cut_off_km=64, n_neighbours=8):
...
def objective(trial):
    ...
    cut_off_km = 2**trial.suggest_int('cut_off_km', 5, 7)
    n_neighbours = trial.suggest_int('n_neighbours', 4, 10)
...
best_params_df.at[0,'cut_off_km'] = 2**best_params_df.loc[0]['cut_off_km']
best_params_df.at[0,'look_up'] = 6*best_params_df.loc[0]['look_up']

best_params_df.to_csv('neighbours_best_params.csv', index=False)
log_file = open(sys.argv[3], "a")
